...
use OpenEMR\RestControllers\LogRestController;
...
RestConfig::$ROUTE_MAP = array(
	...
	"GET /api/log/:username/:startDate/:endDate" => function ($username, $startDate, $endDate) {
        RestConfig::authorization_check("admin", "users");
		EventAuditLogger::instance()->newEvent('REST-Log-GetLogByDate', (new AuthRestController())->getUserFromToken($_SERVER["HTTP_X_API_TOKEN"]), "REST", 1, "GET /api/log/{$username}/{$startDate}/{$endDate}");
        return (new LogRestController())->getLogByDate($username, $startDate, $endDate);
    },
	"GET /api/log/:username" => function ($username) {
        RestConfig::authorization_check("admin", "users");
		/*$headers=array();
		foreach (getallheaders() as $name => $value) {
			$headers[$name] = $value;
		}*/
		EventAuditLogger::instance()->newEvent('REST-Log-GetLog', 'TESTSTESFEHJKHFL', "REST", 1, "GET /api/log/{$username}");
        return (new LogRestController())->getLog($username);
    },
	"GET /api/users" => function () {
        RestConfig::authorization_check("admin", "users");
		//EventAuditLogger::instance()->newEvent('REST-Users-GetAll', (new AuthRestController())->getUserFromToken($_SERVER["HTTP_X_API_TOKEN"]), "REST", 1, "GET /api/users");
        return (new UserRestController())->getAll();
    },
	"GET /api/users/:username" => function ($username) {
        RestConfig::authorization_check("admin", "users");
		//EventAuditLogger::instance()->newEvent('REST-Users-GetAll', (new AuthRestController())->getUserFromToken($_SERVER["HTTP_X_API_TOKEN"]), "REST", 1, "GET /api/users");
        return (new UserRestController())->getOne($username);
    },
	"GET /api/users/authorized/:username" => function ($username) {
        RestConfig::authorization_check("admin", "users");
		//EventAuditLogger::instance()->newEvent('REST-Users-GetAll', (new AuthRestController())->getUserFromToken($_SERVER["HTTP_X_API_TOKEN"]), "REST", 1, "GET /api/users");
        return (new UserRestController())->getAuthorized($username);
    },
	
    "GET /api/facility" => function () {
        RestConfig::authorization_check("admin", "users");
		EventAuditLogger::instance()->newEvent('REST-Facility-GetAll', (new AuthRestController())->getUserFromToken($_SERVER["HTTP_X_API_TOKEN"]), "REST", 1, "GET /api/facility");
        return (new FacilityRestController())->getAll();
    },
    "GET /api/facility/:fid" => function ($fid) {
        RestConfig::authorization_check("admin", "users");
		EventAuditLogger::instance()->newEvent('REST-Facility-GetAll', (new AuthRestController())->getUserFromToken($_SERVER["HTTP_X_API_TOKEN"]), "REST", 1, "GET /api/facility/{$fid}");
